...
| 2026-09-27 | research | 「摘要 6 部分/模式 ≥5 含场景/moodboard 6 维/建议有依据」是文本结构规则，靠目测核对研究 md | `scripts/validate.mjs`（新增 research-summary 校验） | 结构化 md 可执行化：查 6 部分标题、模式表行数 ≥5 + 适用场景列、moodboard 6 维关键词、建议每条须含「模式 N/场景/竞品」依据标注；竞品三层「3 个不凑 4 个」的兼任条款 spec 已有，本轮验证有效 |
| 2026-09-27 | handoff | 「Token 六组齐/标注无裸 hex/资源命名+currentColor」靠目测；资源命名正则首版漏下划线（bg_hero_pattern 误报） | `scripts/validate.mjs`（新增 handoff-pack 校验） | 交付包文本可执行化：tokens.json 合法 JSON+六组键、.pin 标注层禁裸 hex、assets SVG 命名 ^(ic\|bg\|ill)_[a-z0-9_-]+\.svg$ + currentColor；「1:1 误差 0」位图需渲染量测，留产物级 Playwright+SELF-CHECK 记录制 |
| 2026-09-27 | image-prompt | 「必备节齐/负面词 ≥3/品牌 hex 绑定/命令含 --save/可选节缺项须说明」靠目测核对 prompt md | `scripts/validate.mjs`（新增 image-prompt 校验） | Prompt 文件结构可执行化：必备节标题齐、负面词节编号行 ≥3、全文须含品牌 hex 且禁「企业蓝/现代感」模糊词、toapis 命令须含 --save、可选节缺失须有「不适用/缺项」说明；负样本（负面词删到 1 条）触发 ✗ 验证 |
| 2026-09-27 | review | ①审阅 HTML 骨架规则（居中 flexbox/FSAA 双路径/标记校验/localStorage）靠目测；②产物首版 hidden 属性被 .review-overlay{display:flex} 覆盖，遮罩恒在拦截全页点击——「display 覆盖 [hidden]」是同名坑；③校验正则子串陷阱：/showDirectoryPicker/ 匹配改名后的 showDirectoryPickerGone 恒真，负向测试假绿 | `scripts/validate.mjs`（新增 review-mech 校验，编号 V17——research-layers 已占 V16） | ①骨架规则可执行化：fixed+flex 双居中、showDirectoryPicker( 调用形式（括号防子串误配）、IndexedDB 句柄、createObjectURL 降级、role=alert、localStorage、max-width 弹窗宽；②产物修 .review-overlay[hidden]{display:none}——「hidden 被 display 覆盖」属通用坑，register 在 duotone-reset 同类（双态赋值须重置），不另立校验；③负向测试用「删除调用」而非「改名」 |
| 2026-09-27 | spec-customization | 「基线引用/覆盖三要素（字段+新值+原因）/不覆盖显式声明/Token 与 spec 一致/pages 仅差异」靠目测核对 | `scripts/validate.mjs`（新增 spec-custom 校验） | 结构可执行化：MASTER 须含基线引用（brand/web/mobile 三 spec）+覆盖行相邻有「原因」+不覆盖项节；Token 主色与 MASTER 主色 hex 一致性比对（脱节即 ✗）；pages 须含「不覆盖项/沿用」。双负向（删不覆盖节→✗；Token 主色改 #000000→✗ 脱节检出）验证通过 |
| 2026-09-27 | review→image-prompt/spec-customization（插队转办） | critic round2 转办：image-prompt/spec-customization SELF-CHECK 评分-only 摆拍（130 字节）；连带发现 review 主目录同类（76 字节）；V18(b) 原只扫 review-critic-round2 子目录，主目录摆拍不拦 | `scripts/validate.mjs`（V18(b) 扩 scope：全部 SELF-CHECK.md ≥800 字节且 ≥5 条 ✅；修复 readdirSync 对文件抛 ENOTDIR 的守卫顺序） | 三份 SELF-CHECK 全部重写为逐条核对+实测依据；负向（任一目录摆拍→✗）实证；「自检须含逐条核对」由 V18(b) 全域强制，spec 无需加长 |
...
| 2026-09-24 | illustration | 挑刺终验暴露：①色板登记摆拍 P0——SELF-CHECK/HTML 注释宣称「暗色映射已记入色板暗色分支」，全仓溯源不存在任何拾贝色板文档，14 个 hex（7 亮+7 暗）无载体可对照（brand 域登记的是 Cafero 咖啡棕系，illustration 的 #243B4A 蓝灰系与其无关）；②占比宣称摆拍 P1——演示区称「≈37% ≤ 40%」，Playwright 实测 44.9%（svg 82.5/卡 183.9，三视口一致），超 I2 上限；③审阅弹层焦点不闭环 P1（无 aria-modal/Tab 逃逸/Esc 焦点落 BODY，第三次同款）；④反馈草稿不落 localStorage P2（review spec 明文「自动保存到本地，弹窗关闭不丢」，实测 length=0，重开残留只是 Esc 不 reset 的巧合）；⑤上一轮反馈只读对照 UI 缺失 P2 | `specs/illustration/illustration-design.md`（I3 回填 1 句：「已记入色板」必须有载体文件 assets/<品牌>-palette.json 亮/暗成对登记，注释声称而无文件即摆拍；I2 回填 1 句：占比按渲染后实测、各视口达标，不以单一尺寸理论估算为准）；`scripts/validate.mjs`（新增 V13 ill-palette：illustration 域全部 --brand-* hex 须登记于 assets/*-palette.json，负向实证移除文件 28 处触发 ✗） | 「有规则无载体=可谎称合规」——登记类约束必须落文件并脚本对照→V13 固化；弹层 a11y 连续第三轮同款，执行失误但说明 review spec 弹层条款缺 a11y 硬性表述（暂以 V 系校验+逐轮对勾兜底）；clean-room illustration-critic-round2/（shibei-palette.json 9 角色 18 值、占比 svg 88px 实测 39.4%、焦点困笼、草稿持久化+上轮只读对照）修复全部，报告见 design/critic-reports/latest.md |
| 2026-09-24 | cover | 挑刺终验暴露：①小红书标题字号假达标 P0——复核表 146.9px÷预览渲染高 960=15.3% ✓，导出原生高 1440 实际 10.2%<15%；自检短板2把「理论高」修成「渲染高」恰好把错误制度化（预览画布≠导出尺寸，em 基准字号与画布比例错乱）②布局崩坏被 overflow 静默裁切 P0——同根系统性错误：xhs kicker 换行 622px/sub2 691px，内容溢出画布 497px 被 overflow:hidden 裁掉，截图像素带分析文字顶到边缘；③复核表假阳性 P1——强调色列展示原值 #C77B4A 3.5:1 却静态 ✓（em 实际用 #E8A876 5.7:1），「≥4.5:1」判定列硬编码无判别力；④C6 导出文件缺失 P1——SELF-CHECK 勾三平台 PNG 实则无实体；⑤YouTube 标题负 margin 出自设居中安全框 266px P1；⑥弹层 a11y 第四次同款 P2；⑦「每行≤8字（单行除外）」措辞含混 P2 | `specs/cover/cover-design.md`（C4 回填 2 句：分母=封面真实高且与字号同比例基准，预览≠导出时用容器查询单位 cqh 使占比恒等；核心内容完整落画布禁 overflow 静默裁切；C4/C2 回填：复核表 ✓/✗ 须由计算值驱动禁止静态写死）；`scripts/validate.mjs`（新增 V14 cover-metric：cqh 字号≥15%+主/强调色对比公式+三平台导出 PNG 实体，负向实证 h3 降 14cqh+移走 PNG 触发 2 条 ✗） | 「自证型复核」（用产物内静态 ✓/自选分母）不可信——数值验收须脚本独立直算→V14 固化；「比例基准统一」是封面/海报类产物的系统性陷阱，容器查询单位（cqh/cqw）是 CSS 侧正解；clean-room cover-critic-round2/（容器查询重构+Playwright 原生尺寸导出 900×383/1080×1440/1280×720+弹层 a11y）修复全部，报告见 design/critic-reports/latest.md |
| 2026-09-24 | content | 挑刺终验暴露：①可撤销 toast 摆拍 P1——「已升级订阅 · 撤销」的撤销只是文本（pointer-events:none 不可点）且 2.6s 即消失；②术语表无载体 P1——spec 称「与 Token 一样是单一事实来源」但只是 HTML 表格，无 terms.json 结构化载体，全仓一致性检查无从执行（自检记 TODO 未做）；③审阅弹层 a11y 第五次同款 P1（无 aria-modal/焦点不入/无困笼/Esc 不归还）；④4 个演示按钮用 span 假按钮不可聚焦无语义 P2；⑤反馈 localStorage 持久化+上一轮只读对照第三次缺失 P2 | `specs/content/content-design.md` 回填两句（C5：撤销须可点击真入口、可撤销 toast ≥5s；一致性管理：术语表须结构化载体 terms.json，HTML 表格只是展示，检查以载体驱动）；`scripts/validate.mjs` 新增 V15 term-modal（terms.json 驱动禁词全文扫描+确认动作禁词只查按钮文本，弹层 a11y 静态特征 aria-modal/closeModal/Tab 困笼/移焦对 critic-round2 起强制；负向实证：移走 terms.json 与注入「好的」按钮各触发 ✗） | 「提示了入口」≠「入口可用」——可撤销/可跳转等 affordance 文案必须配真行为，spec 把「真入口+时长」写死；「单一事实来源」不能停留在口号，Token 有 palette.json 先例，术语表同理 terms.json；弹层 a11y 连续五轮同款后以 V15 静态特征兜底（新产物从 round2 起强制）；clean-room content-critic-round2/（terms.json 6 术语+真撤销按钮+6s toast+真 button+弹层/持久化/对照全修），报告见 design/critic-reports/latest.md |
| 2026-09-24 | research | 挑刺终验暴露：①三层覆盖摆拍 P1——摘要自称「2 直接 + 1 间接兼标杆」覆盖三层，但竞品表 Apple Notes 标「直接 + 标杆」、全文无任何间接竞品；spec 修订条款「同产品可兼任」被用来凑层（真正间接层=语音备忘录/剪藏等跨品类采集解法缺席）；②风格关键词裸词 P2——「纸感、沉浸、可靠、轻结构」无解释无落点，Phase 1 无法操作化；③研究元信息缺失 P2——无方法/日期/版本/断言可核性注记，「Apple 空状态曾有改善」等断言置信度不可判；④场景设备要素隐含不显式 P2；⑤moodboard 非竞品参考完全缺席 P2（R6 明文要求，自检以「图片型不脚本化」回避） | `specs/research/research-design.md` 回填三处（R2：每层须真实落点，兼任不得使某层整体缺席，找不到跨品类解法如实写暂缺及原因；R6：关键词须带操作性解释+落点，moodboard 定稿前须补齐竞品亮点+非竞品参考；R7：摘要头部附研究元信息——方法/版本/断言可核性/日期）；`scripts/validate.mjs` 新增 V16 research-layers（间接层落点+关键词锚定+元信息，对 critic-round2 起强制；负向实证：删间接行触发 ✗） | 「覆盖 N 层」类宣称须逐层验实体——措辞兼任（「兼标杆」「间接兼标杆」）是结构化宣称里最易藏摆拍的位置，脚本只认层级列的显式落点；研究摘要作为下游输入文档，断言无置信度标注会把不可核信息带进设计决策；clean-room research-critic-round2/（补真间接竞品 iOS 语音备忘录+关键词解释表+元信息块+场景设备显式），报告见 design/critic-reports/latest.md |
| 2026-09-24 | review | 挑刺终验暴露：①FSAA 用户取消即崩 P1——showDirectoryPicker AbortError 未捕获，用户取消目录选择→uncaught pageerror、保存中断无提示不降级；spec 验收只写「不支持 FSAA 时降级」，漏了「用户取消」这个必经分支；②SELF-CHECK 摆拍 P1——注册 ✅ 时自检仅 130 字节（只 5 维度评分），25 条验收零逐条核对零实测记录，V6 只查评分行即放行；连带发现 image-prompt（✅）与 spec-customization 的 SELF-CHECK 同为 76 字节评分-only 摆拍；③弹窗打开背景可滚动 P2；④走查对象为占位色块非真实原型 P2（上游产物链局限，已声明） | `specs/review/review-design.md` 回填验收 1 条（不支持 FSAA **或用户取消 AbortError** 均须降级为下载——取消是必经分支，失败可见或安静降级，不可崩）；`scripts/validate.mjs` 新增 V18 review-fsaa-selfcheck（①含 showDirectoryPicker 的产物须有 AbortError 捕获分支；②critic-round2 起 SELF-CHECK 须 ≥800 字节 ≥5 条 ✅ 逐条核对，评分-only=摆拍；负向实证：移除 catch 触发 ✗） | 「错误处理只写 happy-path 降级」是 async API 接入的经典盲区——AbortError（用户取消）与「不支持」是两种不同分支，取消须静默降级、其它错误失败可见；「自检」若无逐条核对记录就只是仪式——评分表是结论不是证据，V18 把证据密度脚本化；clean-room review-critic-round2/（AbortError catch+lockScroll+完整 25 条逐条核对自检，实测降级下载触发 pageerror=0），报告见 design/critic-reports/latest.md |
| 2026-09-24 | image-prompt | 挑刺终验暴露：①模型输入元信息污染 P1——Prompt 文件混入「> 说明：…不实际调用」等流程元信息与 markdown 结构标题，而命令用 `--prompt "$(cat file)"`，文件全文成为模型输入（元说明/结构噪音全部进 prompt）；spec 未区分「Prompt 文件=文档」与「=模型输入」两种角色；②SELF-CHECK 评分-only 摆拍 P1（上轮转办，76 字节零逐条核对）；③展示环境未声明 P2——「纸感底 #F4F0E8」浅色锁定，浅色大色块直接压暗色空状态是常见错配，spec 无「生成前声明展示环境」条款 | `specs/image-prompt/image-prompt-design.md` 回填 2 条+验收 2 条（Prompt 文件即模型输入全文，元信息不得进文件放 SELF-CHECK；生成前声明展示环境浅/暗色）；`scripts/toapis.py` 新增 `--prompt-file`（image/video 两子命令，与 --prompt 二选一互斥校验）；`scripts/validate.mjs` 新增 V19 prompt-purity（正文含元信息→✗、缺展示环境声明→✗；双负向实证均触发） | 「生成物输入文件」与「人类阅读文档」混在同一文件是 prompt 工程经典坑——模型会把元说明当指令执行；文件输入通道（--prompt-file）使「文件即输入」契约显式化；展示环境声明把「生成的图用在哪种 UI 环境」前置到生成前；clean-room image-prompt-critic-round2/（纯输入正文+环境声明+7 条逐条核对自检），报告见 design/critic-reports/latest.md |
| 2026-09-24 | spec-customization | 挑刺终验暴露：①基线引用全部断链 P1——MASTER 引 `design/specs/brand/...` 四处，`design/specs/` 不存在（实际 `design-me/specs/`），Override 检索链（pages>MASTER>全局）根基断裂；②「沿用不覆盖」声明与 Token 实值矛盾 P1——radius sm6/md12/lg20 不在基线刻度 [4,6,10,16,999]（md/lg 凭空档位，MASTER 还凭空引用「基线 md=12」）、motion normal 240ms vs 基线 250ms、shadow 自拟暖棕 rgba vs 基线墨蓝 oklch——**声明不覆盖的 Token 实际被覆盖且未记原因**（三处隐性覆盖）；③对比度声称无源 P1——pages/home.md 称「暗化 45% 实测 ≈7.2:1」，WCAG 公式实算 10.4:1，数字编造（若声称贴线值如 4.6:1 就会放行不达标设计，cover 轮「判定列硬编码」同款复发）；④压暗底色 #4A3826 游离未登记 Token P2 | `specs/spec-customization/spec-customization.md` 回填 1 条+验收 2 条（**沿用=逐值一致**：刻度档位/时长/色值引用与基线一致，凭空档位/相近不同值/自拟色调均属隐性覆盖须转显式+原因，基线引用须真实可达路径；可计算主张附计算式与中间值禁无源数字）；`scripts/validate.mjs` 新增 V20 custom-baseline（基线引用路径可达校验+「沿用」Token 与基线 diff——radius 刻度⊆基线、motion 与基线 note 相符；双负向实证：断链/刻度外档位均触发 ✗） | 「沿用」不等于「大概齐」——不覆盖声明会让维护者跳过逐值核对，恰是隐性覆盖最易藏身处（凭空档位甚至被写成「直接消费基线值」）；引用可达性是检索链前提，断链引用让 Override 回溯形同虚设；「实测 ≈X:1」无计算式即摆拍——可计算主张的价值就在可复算；clean-room spec-customization-critic-round2/（路径修正+Token 对齐基线+实算 10.4:1+游离值登记），报告见 design/critic-reports/latest.md |
| 2026-09-24 | handoff | 挑刺终验暴露：①断点声明与实现脱节 P1——边界条件表声明「<480px CTA 独占一行、>1280px 容器 1200 居中」但 CSS 无对应 @media（flex 布局无 480 断点），Playwright 390px 实测 ctaBelow=false——交付标注写了开发要做的行为而实现不存在，照标注还原会得出与实际组件不符的结果（handoff 域最典型的还原偏差源，恰是 spec 开篇要消除的）；②error 变体不可消费 P2——状态表 error 态用内联 style 复现而非真实 class，开发抄不到状态类名，状态表形同视觉示意非交付规格 | `specs/handoff/handoff-design.md` H6 回填验收 1 条（边界条件中声明的响应式断点行为须有对应 @media 实现锚点，声明无实现=脱节）；`scripts/validate.mjs` 新增 V21 handoff-breakpoint（声明的 480/768/1280 断点须有 @media 锚点，负向实证：注入无实现断点声明触发 ✗） | 「注明」边界条件只产出描述，「实现」边界条件才产出可验收组件——描述与实现的落差在交付场景被放大（标注是开发的唯一事实来源）；状态表是规格不是示意图，状态可消费=有类名/token 名可抄；round2 实现双断点（390px ctaBelow=true、1300px 容器 1200）+btn--error class；clean-room handoff-critic-round2/，报告见 design/critic-reports/latest.md。**至此 ✅ 队列清空，15/15 域全部 🛡️** |
